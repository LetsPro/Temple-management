...
import { Calendar, Heart, Bell, BookOpen, ChevronRight, Clock, CheckCircle } from 'lucide-react'
...
export default function DevoteeDashboard() {
  ...
  useEffect(() => {
    if (!user) return
    async function load() {
      const [bookingsRes, donationsRes, announcementsRes, notifRes] = await Promise.all([
        supabase
          .from('bookings')
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-gradient-to-r from-vermilion-700 to-saffron-500 rounded-3xl p-6 text-white">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Link to="/donate" className="inline-flex items-center gap-1.5 bg-white/15 border border-white/30 text-white font-semibold text-sm px-4 py-2 rounded-xl hover:bg-white/20 transition-all">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Recent Donations', value: recentDonations.length, icon: '💛', href: '/portal/donations' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-temple-text flex items-center gap-2">
              <Heart size={16} className="text-saffron-500" /> Recent Donations
            </h3>
            <Link to="/portal/donations" className="text-sm text-vermilion-600 font-medium hover:text-vermilion-700">View all →</Link>
          </div>
          {loading ? (
            ...
            <div className="text-center py-6">
              <div className="text-3xl mb-2">💛</div>
              <p className="text-temple-muted text-sm mb-3">No donations yet</p>
              <Link to="/donate" className="btn-primary text-sm py-2">Make a Donation</Link>
            </div>
          ) : (
            <div className="space-y-3">
              {recentDonations.map(d => (
                <div key={d.id} className="flex items-center gap-3 p-3 bg-cream-100/60 rounded-xl">
                  <div className="w-10 h-10 rounded-xl bg-gold-50 flex items-center justify-center flex-shrink-0 text-lg">🪔</div>
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-temple-text text-sm">{d.purpose}</div>
                    <div className="text-xs text-temple-muted">{format(new Date(d.created_at), 'dd MMM yyyy')} · #{d.donation_number}</div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="font-bold text-vermilion-700 text-sm">₹{d.amount.toLocaleString('en-IN')}</div>
                    <div className={`text-xs font-medium ${d.payment_status === 'paid' ? 'text-green-600' : 'text-amber-600'}`}>{d.payment_status}</div>
                  </div>
                </div>
              ))}
            </div>
