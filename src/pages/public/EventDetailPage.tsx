          ...
          {canRegister && !registered && event.pricing_type === 'paid' && (
            <div className="mb-4 rounded-2xl border border-temple-border p-3">
              ...
              {availableMarkets.length > 1 && <div className="grid grid-cols-2 rounded-lg bg-cream-100 p-1 mb-3" role="tablist" aria-label="Plan region">
                {availableMarkets.map(market => <button key={market} type="button" role="tab" aria-selected={planMarket === market} onClick={() => selectMarket(market)} className={`rounded-md px-2 py-1.5 text-xs font-semibold transition-colors ${planMarket === market ? 'bg-vermilion-700 text-white shadow-sm' : 'text-temple-muted hover:text-temple-text'}`}>
